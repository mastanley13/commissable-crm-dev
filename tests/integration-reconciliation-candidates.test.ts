...
integrationTest("REC-AUTO-16B: business Account ID and distributor-side Other Order ID drive exact matches", async ctx => {
  const dbModule = await import("../lib/db")
  const prisma = (dbModule as any).prisma ?? (dbModule as any).default?.prisma
  ...
  const opportunity = await prisma.opportunity.create({
    data: {
      tenantId: ctx.tenantId,
      accountId: ctx.distributorAccountId,
      name: "Business ID Opportunity",
      accountIdHouse: "HOUSE-ACC-100",
      orderIdDistributor: "ORD-DIST-100",
      distributorName: "Test Distributor",
      vendorName: "Test Vendor",
    },
    select: { id: true },
  ...
  const deposit = await prisma.deposit.create({
    data: {
      ...
      status: "InReview",
      depositName: "Business ID Confidence Deposit",
      paymentDate: new Date("2026-02-15T00:00:00Z"),
      distributorAccountId: ctx.distributorAccountId,
      vendorAccountId: ctx.vendorAccountId,
      createdByUserId: ctx.userId,
    },
  ...
  const line = await prisma.depositLineItem.create({
    data: {
      ...
      commissionUnallocated: 10,
      vendorAccountId: ctx.vendorAccountId,
      accountIdVendor: "HOUSE-ACC-100",
      orderIdVendor: "ORD-DIST-100",
      vendorNameRaw: "Test Vendor",
      distributorNameRaw: "Test Distributor",
    },
    select: { id: true },
  ...
  const schedule = await prisma.revenueSchedule.create({
    data: {
      ...
      vendorAccountId: ctx.vendorAccountId,
      opportunityId: opportunity.id,
      scheduleNumber: "RS-BUSINESS-ID-1",
  ...
  assert.ok(Array.isArray(payload.data))
  const candidate = payload.data!.find(row => row.id === schedule.id)
  assert.ok(candidate, "Expected schedule with matching business Account ID and Other Order ID to appear")
  assert.equal(candidate.matchType, "exact")
  assert.ok(candidate.signals.some((signal: any) => signal.id === "account_id_exact" && signal.score === 1))
  assert.ok(candidate.signals.some((signal: any) => signal.id === "order_id_exact" && signal.score === 1))
})
