...
const { rootDir, resolveSuiteRunDir } = require('./reconciliation-suite-paths.cjs')
...
async function main() {
  ...
  const runDir = resolveSuiteRunDir(requestedRunId)
  ...
  console.log(`Opening reconciliation suite report from ${htmlDir}`)
  console.log(`Using http://127.0.0.1:${port}`)

  const result = spawnSync(
    process.execPath,
    [playwrightCliPath, 'show-report', htmlDir, '--host', '127.0.0.1', '--port', String(port)],
    {
      cwd: rootDir,
      stdio: 'inherit',
    }
