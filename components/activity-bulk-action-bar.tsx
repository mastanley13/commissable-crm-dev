...
import { Download, Trash2, ToggleLeft, UserCog } from "lucide-react"
import { cn } from "@/lib/utils"

interface ActivityBulkActionBarProps {
  count: number
  disabled?: boolean
  onSoftDelete: () => void
  onExportCsv: () => void
...
export function ActivityBulkActionBar({
  count,
  disabled = false,
  onSoftDelete,
  onExportCsv,
  ...
  return (
    <div className={cn("fixed inset-x-0 bottom-6 z-50 pointer-events-none px-4", className)}>
      <div
        ...
        role="region"
        aria-label="Activity bulk actions"
      >
        <div className="text-sm font-medium text-blue-900">
          {count} activity{count === 1 ? "" : "ies"} selected
        </div>
