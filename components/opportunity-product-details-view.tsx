...
interface ProductHeaderProps {
  product: OpportunityProductDetailRecord
  ...
  return (
    <div className="flex flex-col gap-0">
      ...
      <div className="rounded-t-2xl bg-gray-100 p-3 shadow-sm h-[300px] overflow-y-auto">
        <div className="mb-2 flex items-center justify-between">
          <div className="space-y-1">
            <p className="text-[13px] font-semibold uppercase tracking-wide text-primary-600">Opportunity Product Detail</p>
      ...
      {activeTab === "history" && (
        <div className="rounded-b-2xl border-x border-b border-gray-200 bg-white pt-0 px-3 pb-4">
          <div className="border-t-2 border-t-primary-600 -mr-3 pt-3">
            <AuditHistoryTab entityName="OpportunityProduct" entityId={product.id} description={TAB_DESCRIPTIONS.history} />
          </div>
        </div>
      )}
    </div>
...
export function OpportunityProductDetailsView({
  lineItem,
  loading,
  error,
  onEdit,
  ...
  const submitProduct = useCallback(
    async (patch: Partial<ProductInlineForm>, draft: ProductInlineForm) => {
      if (!product?.id) {
        throw new Error("Opportunity product ID is required")
      }
      ...
      try {
        const response = await fetch(`/api/opportunities/line-items/${product.id}`, {
          method: "PATCH",
        ...
        if (!response.ok) {
          const message = body?.error ?? "Failed to update opportunity product"
        ...
        showSuccess("Opportunity product updated", "Changes saved.")
        await onRefresh?.()
        return draft
      } catch (err) {
        const message = err instanceof Error ? err.message : "Unable to update opportunity product"
        showError("Failed to update opportunity product", message)
        throw err
      }
    },
  ...
  if (loading) {
    return (
      <div className="flex min-h-[420px] flex-col items-center justify-center gap-3 text-sm text-gray-500">
        ...
        Loading opportunity product details...
      </div>
    )
  }
  ...
  if (error) {
    return (
      <div className="flex min-h-[420px] flex-col items-center justify-center gap-4 rounded-2xl border border-rose-200 bg-rose-50 p-8 text-center text-sm text-rose-700">
        <p className="text-base font-semibold text-rose-800">Unable to load opportunity product details</p>
        <p>{error}</p>
        {onRefresh ? (
          <button
            type="button"
  ...
  if (!product) {
    return (
      <div className="flex min-h-[420px] flex-col items-center justify-center gap-3 rounded-2xl border border-gray-200 bg-white p-8 text-center text-sm text-gray-500">
        <p className="text-base font-semibold text-gray-900">Opportunity product not found</p>
