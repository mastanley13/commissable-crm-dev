...
- **1.1 Implement the final field priority order** - **Partially implemented (MVP in place).** Hierarchical Pass A/B scoring, strong ID and account legal name checks, product name/part number similarity, and FIFO tie-breaking are coded in lib/matching/deposit-matcher.ts and wired into the `/candidates` API; Location ID / Customer PO # and Product Description are not yet used. The hierarchical engine is gated behind HIERARCHICAL_MATCHING_ENABLED / NEXT_PUBLIC_HIERARCHICAL_MATCHING and can be toggled per-request via the dev-only "Matching Dev Controls" on the Reconciliation page (Legacy vs Hierarchical).
...
- **3.1 Global variance setting (per tenant)** - **Not implemented.** There is no reconciliation settings model or UI; only the varianceTolerance option on matchDepositLine.
- **3.1 Use variance for auto-apply vs suggest vs FLEX** - **Partially implemented in the engine only.** Pass A uses varianceTolerance to decide whether a candidate qualifies as a 1.0 "perfect" match, but there is no orchestration that turns this into auto-apply vs suggest vs FLEX behavior.
- **3.2 Auto-match pass over Unmatched lines** - **Not implemented.** There is no service or job that iterates deposit lines and automatically applies matches; all matches today are manual via the per-line apply-match endpoint.
