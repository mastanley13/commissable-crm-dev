...
const BASE_COLUMNS: Column[] = [
  ...
  },
  {
    ...
    width: 180,
    minWidth: calculateMinWidth({ label: 'Other - Account ID', type: 'text', sortable: false }),
    maxWidth: 260,
    accessor: 'accountIdVendor',
    render: (_value, row: OpportunityRow) => row.accountIdVendor ?? row.accountIdDistributor ?? '',
    hidden: true,
  },
  {
    id: 'customerIdVendor',
    ...
    width: 180,
    minWidth: calculateMinWidth({ label: 'Other - Customer ID', type: 'text', sortable: false }),
    maxWidth: 260,
    accessor: 'customerIdVendor',
    render: (_value, row: OpportunityRow) => row.customerIdVendor ?? row.customerIdDistributor ?? '',
    hidden: true,
  },
  {
    id: 'otherSource',
  ...
  const handleBulkExportCsv = useCallback(() => {
    ...
    rows.forEach((row) => {
        const cells = [
          ...
          row.orderIdHouse ?? '',
          row.accountIdVendor ?? row.accountIdDistributor ?? '',
          row.customerIdVendor ?? row.customerIdDistributor ?? '',
          row.locationId ?? '',
