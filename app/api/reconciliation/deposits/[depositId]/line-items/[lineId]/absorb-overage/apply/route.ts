...
export async function POST(
  ...
  return withPermissions(request, ["reconciliation.manage"], async req => {
    ...
    await logAudit({
      ...
      metadata: {
        action: "ApplyUsageVarianceAdjustment",
        route: "absorb-overage/apply",
        depositId,
        revenueScheduleId,
        applyToFuture,
        usageOverage: result.usageOverage,
        commissionOverage: result.commissionOverage,
        updatedScheduleIds: result.updatedScheduleIds,
        futureUpdatedScheduleIds: result.futureUpdatedScheduleIds,
        createdAdjustmentIds: result.createdAdjustmentIds,
      },
    })
